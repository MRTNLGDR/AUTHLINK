use authlink_contracts::{
    ...
    GuardianSignals, OnboardingProgress, OnboardingStep, OnboardingStepId, RiskLevel, StepStatus,
};
use axum::{
    extract::State,
    http::StatusCode,
...
fn evaluate(signals: &GuardianSignals) -> GuardianDecision {
    ...
    let (level, action, step_up) = match score {
    ...
    if signals.device_integrity_penalty > 0 { reasons.push("integridade do dispositivo".into()); }
    if signals.network_penalty > 0 { reasons.push("postura de rede".into()); }
    if signals.credential_exposure_penalty > 0 { reasons.push("exposição de credencial".into()); }
    if signals.session_context_penalty > 0 { reasons.push("contexto de sessão".into()); }
    if signals.anomaly_penalty > 0 { reasons.push("anomalia comportamental".into()); }
    if signals.strong_auth_credit > 0 { reasons.push("crédito por autenticação forte".into()); }
    if reasons.is_empty() { reasons.push("nenhum sinal de risco relevante".into()); }
